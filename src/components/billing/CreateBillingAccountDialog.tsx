...
export function CreateBillingAccountDialog({
  ...
  const validateForm = () => {
    ...
    if (!billingDate) {
      toast({
        title: "Error",
        description: "Debe seleccionar la fecha de facturación",
        variant: "destructive"
      });
      return false;
    }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Check if billing account for this date already exists
      const billingDateString = format(billingDate!, 'yyyy-MM-dd');
      const { data: existingBilling } = await supabase
        .from('billing_accounts')
        .select('id')
        .eq('contract_id', selectedContract)
        .eq('billing_month', billingDateString)
        .maybeSingle();

      if (existingBilling) {
        toast({
          title: "Error",
          description: "Ya existe una cuenta de cobro para este contrato en la fecha seleccionada",
          variant: "destructive"
        });
        return;
      }
      ...
      const { data: billingAccount, error: billingError } = await supabase
        ...
        .insert({
          ...
          billing_month: billingDateString,
      ...
      const uploadPromises = [
        ...
        // Save activity report as document
        (async () => {
          const reportBlob = new Blob([activityReport], { type: 'text/plain' });
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-6">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    ...
                    {billingDate ? format(billingDate, "dd/MM/yyyy") : "Seleccionar fecha"}
                  </Button>
                </PopoverTrigger>
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Describa detalladamente las actividades, entregables y logros del período de facturación.
            </p>
          </div>
