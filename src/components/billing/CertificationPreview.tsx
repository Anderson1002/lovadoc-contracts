...
export function CertificationPreview({
  ...
  const handleExportPDF = async () => {
    ...
    doc.setFontSize(9);
    doc.setFont(undefined, 'bold');
    doc.text(`1. SERVICIOS Y/O PRODUCTOS RECIBIDOS A SATISFACCIÓN CORRESPONDIENTES AL PERIODO DEL MES DE ${certificationMonth || '___'} DE ${currentYear}.`, contentLeftMargin, yPosition, { maxWidth: contentTextWidth });
    yPosition += 8;
    doc.setFont(undefined, 'normal');
    ...
    doc.text(splitSection1, contentLeftMargin, yPosition);
    ...
    const splitNota1 = doc.splitTextToSize(nota1Text, contentTextWidth - 14);
    doc.text(splitNota1, contentLeftMargin + 14, yPosition);
    ...
    const splitNota2 = doc.splitTextToSize(nota2Text, contentTextWidth - 14);
    doc.text(splitNota2, contentLeftMargin + 14, yPosition);
    yPosition += splitNota2.length * 4 + 6;
    
    // Section 2: Novedades
    checkNewPage(25);
    doc.setFont(undefined, 'bold');
    doc.text('2. NOVEDADES O SITUACIONES ANORMALES PRESENTADAS DURANTE EL DESARROLLO DEL CONTRATO.', contentLeftMargin, yPosition, { maxWidth: contentTextWidth });
    yPosition += 6;
    doc.setFont(undefined, 'normal');
    const splitNovedades = doc.splitTextToSize(novedadesTexto, contentTextWidth);
    doc.text(splitNovedades, contentLeftMargin, yPosition);
    ...
    doc.text(splitSection3Title, contentLeftMargin, yPosition);
    ...
    doc.text(splitSection3Sub, contentLeftMargin, yPosition);
    ...
    doc.text(splitSection3, contentLeftMargin, yPosition);
    yPosition += splitSection3.length * 4 + 6;
    
    // Section 4: Risk Matrix
    checkNewPage(35);
    doc.setFont(undefined, 'bold');
    doc.text('4. ACTIVIDADES DE TRATAMIENTO Y MONITOREO A LA MATRIZ DE RIESGO DEL CONTRATO.', contentLeftMargin, yPosition, { maxWidth: contentTextWidth });
    yPosition += 6;
    doc.setFont(undefined, 'normal');
    ...
    doc.text(splitSection4, contentLeftMargin, yPosition);
