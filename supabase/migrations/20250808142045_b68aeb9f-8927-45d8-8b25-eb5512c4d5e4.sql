...
STABLE
AS $$
    SELECT cs.code, cs.name, cs.display_name, cs.description
    FROM public.contract_states cs
    WHERE cs.code = state_code;
$$;

-- Crear trigger para sincronizar estado de billing_accounts
CREATE OR REPLACE FUNCTION public.sync_billing_account_state()
RETURNS TRIGGER AS $$
BEGIN
    -- Si se actualiza state_code, actualizar status
    IF NEW.state_code IS DISTINCT FROM OLD.state_code THEN
        SELECT name INTO NEW.status 
        FROM public.billing_account_states 
        WHERE code = NEW.state_code;
    END IF;
    
    -- Si se actualiza status, actualizar state_code
    IF NEW.status IS DISTINCT FROM OLD.status THEN
        SELECT code INTO NEW.state_code 
        FROM public.billing_account_states 
        WHERE name = NEW.status;
    END IF;
...
CREATE TRIGGER sync_billing_account_state_trigger
    BEFORE UPDATE ON public.billing_accounts
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_billing_account_state();

-- Crear trigger para sincronizar estado de contracts
CREATE OR REPLACE FUNCTION public.sync_contract_state()
RETURNS TRIGGER AS $$
BEGIN
    -- Si se actualiza state_code, actualizar estado
    IF NEW.state_code IS DISTINCT FROM OLD.state_code THEN
        SELECT name INTO NEW.estado 
        FROM public.contract_states 
        WHERE code = NEW.state_code;
    END IF;
    
    -- Si se actualiza estado, actualizar state_code
    IF NEW.estado IS DISTINCT FROM OLD.estado THEN
        SELECT code INTO NEW.state_code 
        FROM public.contract_states 
        WHERE name = NEW.estado;
    END IF;
...
CREATE TRIGGER sync_contract_state_trigger
    BEFORE UPDATE ON public.contracts
    FOR EACH ROW
    EXECUTE FUNCTION public.sync_contract_state();